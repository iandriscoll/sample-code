...
def balanced(m):
    ...
    False
    """
    check = [balanced(end(side)) for side in sides(m) if not is_weight(end(side))]
    if False in check:
        return False
    elif label(sides(m)[0]) * total_weight(end(sides(m)[0])) != label(sides(m)[1]) * total_weight(end(sides(m)[1])):
        return False
    return True
#######
# OOP #
...
def make_withdraw(balance, password):
    ...
    """
    incorrect = []
    def secure(amount, guess):
        nonlocal password, balance, incorrect
        if password == guess and len(incorrect) < 3:
            if amount > balance:
                return 'Insufficient funds'
            else:
            ...
            if len(incorrect) < 3:
                ...
                return "Your account is locked. Attempts: " + repr(incorrect)
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(test) == str:
        return test
    def funct(x, password):
        if old_password == password or new_password == password:
            return withdraw(x, old_password)
        return withdraw(x, password)
    return funct
