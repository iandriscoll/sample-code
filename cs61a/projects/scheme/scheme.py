...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        function = scheme_eval(expr.first, env)
        check_procedure(function)
        argus = expr.second.map(lambda x: scheme_eval(x, env))
        return scheme_apply(function, argus, env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        exprs = Pair(target.second, expressions.second)
        env.define(target.first, do_lambda_form(exprs, env))
        return target.first
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    x = LambdaProcedure(formals, body, env)
    return x
    # END PROBLEM 9
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        check_form(x, 2, 2)
        f = x.first
        v = scheme_eval(x.second.first, env)
